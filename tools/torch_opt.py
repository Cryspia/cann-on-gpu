...
P0=torch.from_numpy(np.fromfile(pre+".p",dtype=np.float32).astype(np.float64))
G =torch.from_numpy(np.fromfile(pre+".g",dtype=np.float32).astype(np.float64))
def save(t): t.detach().numpy().astype(np.float32).tofile(pre+".out")
def via(make):
...
else: sys.stderr.write("torch_opt: no ref %s\n"%op); sys.exit(2)
