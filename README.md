...
- **[`cuda/TODO.md`](cuda/TODO.md)** — Remaining roadmap: operator coverage is functionally complete on this machine (~98%), so the file tracks only work bound to hardware this box lacks — real-Ascend golden cross-checks and bit-exact format fidelity, the performance backlog whose gains GB10's UMA masks, and multi-card / multi-node scaling.
