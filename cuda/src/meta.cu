...
// Native tensor list: standard aclTensorList input for Cat/Split etc. (signature matches acl_meta.h)
...
// Retrieve the element at index from the list (standard CANN pattern: consumer accesses TensorList by index, e.g. per-expert weights)
